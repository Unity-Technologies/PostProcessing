...
namespace UnityEngine.Experimental.PostProcessing
{
    ...
    public sealed class VignetteRenderer : PostProcessEffectRenderer<Vignette>
    {
        public override void Render(PostProcessRenderContext context)
        {
            ...
            sheet.properties.SetColor(Uniforms._Vignette_Color, settings.color.value);

            if (settings.mode == VignetteMode.Classic)
            {
                sheet.properties.SetFloat(Uniforms._Vignette_Mode, 0f);
                sheet.properties.SetVector(Uniforms._Vignette_Center, settings.center.value);
                float roundness = (1f - settings.roundness.value) * 6f + settings.roundness.value;
                sheet.properties.SetVector(Uniforms._Vignette_Settings, new Vector4(settings.intensity.value * 3f, settings.smoothness.value * 5f, roundness, settings.rounded.value ? 1f : 0f));
            }
            else // Masked
            {
                sheet.properties.SetFloat(Uniforms._Vignette_Mode, 1f);
                sheet.properties.SetTexture(Uniforms._Vignette_Mask, settings.mask.value);
                sheet.properties.SetFloat(Uniforms._Vignette_Opacity, Mathf.Clamp01(settings.opacity.value));
            }
        }
    }
}
